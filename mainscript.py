...
def repeated_action():
    ...
    if cells[f'({x}, {y})'] == "forge":
        if Forge.forge_unlocked == False:
            ...
            choice = input("What would you like to do? (move, interact, use, forge, inventory, stats, skip, exit): ").lower().strip()
    elif Forge.forge_unlocked == True:
        choice = input("What would you like to do? (move, interact, use, forge, inventory, stats, skip, exit): ").lower().strip()
    else:
        choice = input("What would you like to do? (move, interact, use, inventory, stats, skip, exit): ").lower().strip()
        #print(cells[(1, 0)], x, y)
    if choice == "exit":
    ...
    while True:
        ...
        try:
            if music.is_alive():
                ...
                music = Process(target=play_sound, daemon=True)
                music.start()
        except Exception:
            music = Process(target=play_sound, daemon=True)
            music.start()
